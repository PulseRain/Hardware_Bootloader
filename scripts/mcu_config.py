...
class MCU_OCD:
    ...
    def _verify_crc (self, data):
        ...
        crc_data = MCU_OCD._crc16_ccitt.get_crc (data_list [0 : MCU_OCD._OCD_DEBUG_FRAME_REPLY_LEN - 2])
     
        if (crc_data == data_list [MCU_OCD._OCD_DEBUG_FRAME_REPLY_LEN - 2 : MCU_OCD._OCD_DEBUG_FRAME_REPLY_LEN]):
            return True
        else:
            return False
    ...
    def uart_select (self, ocd1_cpu0):
        ...
        frame_type_byte = MCU_OCD._OCD_DEBUG_TYPE_UART_SEL * 2 + MCU_OCD._toggle;
        MCU_OCD._toggle = 1 - MCU_OCD._toggle
            
        frame = MCU_OCD._OCD_DEBUG_SYNC + [frame_type_byte] + [0x99, 0x12, 0x34, 0xab, 0xcd, 0xab, ocd1_cpu0*2]
        frame = frame + MCU_OCD._crc16_ccitt.get_crc (frame)
    ...
    def cpu_reset (self, show_crc_error=0):
        ...
        condition = True
        while (condition):
            frame_type_byte = MCU_OCD._OCD_DEBUG_TYPE_CPU_RESET_WITH_ACK * 2 + MCU_OCD._toggle;
            MCU_OCD._toggle = 1 - MCU_OCD._toggle
            
            frame = MCU_OCD._OCD_DEBUG_SYNC + [frame_type_byte] + [0x88, 0x12, 0x34, 0xab, 0xcd, 0xab, 0xcd]
            frame = frame + MCU_OCD._crc16_ccitt.get_crc (frame)
            ...
            self._serial.write (frame)
            ret = self._serial.read (MCU_OCD._OCD_DEBUG_FRAME_REPLY_LEN)
    ...
    def mem_read_32bit (self, addr, show_crc_error=0):
        ...
        while (condition):
            ...
            frame_type_byte = MCU_OCD._OCD_DEBUG_TYPE_PRAM_READ_4_BYTES * 2 + MCU_OCD._toggle;
            MCU_OCD._toggle = 1 - MCU_OCD._toggle
            
            frame = MCU_OCD._OCD_DEBUG_SYNC + [frame_type_byte] + [addr_write_high_high_byte, addr_write_high_byte, addr_write_low_byte]
        ...
        if (self._verbose):
            print ("receive: ", [hex(i) for i in ret])
            print ("====> ", [hex(i) for i in ret[MCU_OCD._OCD_DEBUG_FRAME_REPLY_LEN - 6 : MCU_OCD._OCD_DEBUG_FRAME_REPLY_LEN - 2]])
        
        
        return [i for i in ret[MCU_OCD._OCD_DEBUG_FRAME_REPLY_LEN - 6 : MCU_OCD._OCD_DEBUG_FRAME_REPLY_LEN - 2]]
    ...
    def mem_write_32bit (self, addr, data, ack=1, show_crc_error=0):
        ...
        #print ("wr32bit, addr = 0x%x data = 0x%x" % (addr, data))
        ...
        while (condition):
            data_in = data
            if (ack):
                frame_type_byte = MCU_OCD._CONFIG_TYPE_PRAM_WRITE_4_BYTES_WITH_ACK * 2 + MCU_OCD._toggle
            else:
                frame_type_byte = MCU_OCD._CONFIG_TYPE_PRAM_WRITE_4_BYTES_WITHOUT_ACK * 2 + MCU_OCD._toggle
            
            MCU_OCD._toggle = 1 - MCU_OCD._toggle
            
            frame = MCU_OCD._CONFIG_SYNC + [frame_type_byte] + [addr_write_high_high_byte, addr_write_high_byte, addr_write_low_byte]
    ...
    def mem_write_128byte (self, addr, data_list, show_crc_error=0):
        ...
        condition = True
        #print ("wr128, addr = 0x%x " % (addr))
        
        while (condition):
            frame_type_byte = MCU_OCD._CONFIG_TYPE_PRAM_WRITE_128_BYTES_WITH_ACK * 2 + MCU_OCD._toggle
            MCU_OCD._toggle = 1 - MCU_OCD._toggle
    ...
    def _write_mem (self, addr, data):
        ...
        for i in range (total_words - total_128byte_frame * 32):
            ...
            self.mem_write_32bit(addr + offset, data_int, 1, 1)
            offset = offset + 4
    ...
    def start_to_run (self, start_address, show_crc_error=0):
        ...
        while (condition):
            ...
            frame_type_byte = MCU_OCD._OCD_DEBUG_TYPE_COUNTER_CONFIG * 2 + MCU_OCD._toggle;
            MCU_OCD._toggle = 1 - MCU_OCD._toggle
            
            
                  
            frame = MCU_OCD._OCD_DEBUG_SYNC + [frame_type_byte] + [0x77, 0x12, 0x34, (start_address >> 24) & 0xFF, (start_address >> 16) & 0xFF, (start_address >> 8) & 0xFF, (start_address >> 0) & 0xFF]
            frame = frame + MCU_OCD._crc16_ccitt.get_crc (frame)
    ...
    def __init__ (self, com_port, baud_rate, verbose=0):
        self._serial = serial.Serial(com_port, baud_rate, timeout=MCU_OCD._OCD_SERIAL_TIME_OUT)
...
if __name__ == "__main__":
    ...
    for opt, args in opts:
        if opt in ('-b', '--baud'): 
            ...
            print ("     python mcu_config.py --port=COM9 --reset --image=C:\GitHub\Reindeer\bitstream_and_binary\zephyr\hello_world.elf --console_enable --run")
    ...
    try:
        ocd = MCU_OCD (com_port, baud_rate, verbose=0)
